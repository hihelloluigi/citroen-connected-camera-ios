...
public final class MediaListViewModel {
	...
	private let onAction: NavigationActionHandler<GalleryNavigationAction>
	public private(set) var state: LoadState<[MediaEntity]> = .idle
	public private(set) var status: CameraStatusEntity?
	...
	public init(repository: any GalleryRepositoryProtocol,
				photoSaver: any PhotoLibrarySaverProtocol,
				onAction: @escaping NavigationActionHandler<GalleryNavigationAction>) {
		self.repository = repository
		self.photoSaver = photoSaver
		self.onAction = onAction
	}

	/// The user tapped a cell outside select mode. The view model decides nothing about where that
	/// goes — it reports the intent and the coordinator routes it.
	public func select(_ item: MediaEntity) {
		guard !isSelecting else { return }
		onAction(.mediaTapped(item))
	}

	/// The currently-loaded items, or empty when idle/loading/failed.
	public var items: [MediaEntity] {
	...
	public func refresh() async {
		do {
			let media = try await repository.media()
	...
	public func snapshot() async {
		do {
			let item = try await repository.snapshot()
	...
	public func deleteSelected() async {
		...
		isSelecting = false
		do {
			try await repository.delete(targets)
			await refresh()
		} catch {
	...
	public func downloadSelected() async {
		...
		for item in targets {
			...
			do {
				...
				_ = try await repository.download(item, to: destination, progress: { _ in })
